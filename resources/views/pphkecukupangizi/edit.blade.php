  ...
  <div class="card shadow mb-4">
      ...
      <div class="collapse show" id="collapseCardExample">
        <div class="card-body">
          ...
          <form action="{{route('pphkecukupangizi.update',[$pphkecukupangizi->id])}}" method="POST">
            @csrf
	    @method('PUT')
                <div class="row">
                <div class="col-12">
                    ...
                    <tr> 
                      ...
                      <td>
                        <div class="input-group">
                          <label for="">{{$b->bahanpanganid->bahanpangan}}</label>
                        </div>
                      </td> 
                      <td>
                              <div class="input-group">
                                <input type="text" name="pphkecukupangizi[{{$loop->index}}][kkal]" value="{{$b->kkal}}" placeholder="kkal/kapita" class="form-control {{$errors->first('kkal') ? "is-invalid" : ""}}" aria-describedby="basic-addon2">
                              </div>
                              <div class="invalid-feedbeck"> {{$errors->first('pphkecukupangizi.' . $loop->index . '.kkal')}}</div>
                            </td> 
                            <td>
                              <div class="input-group">
                                <input type="text" name="pphkecukupangizi[{{$loop->index}}][gram]" value="{{$b->gram}}" placeholder="gram/kapita" class="form-control {{$errors->first('gram') ? "is-invalid" : ""}}" aria-describedby="basic-addon2">
                              </div>
                              <div class="invalid-feedbeck"> {{$errors->first('pphkecukupangizi.' . $loop->index . '.gram')}}</div>
                            </td> 
                            <td>
                              <div class="input-group">
                                <input type="text" name="pphkecukupangizi[{{$loop->index}}][tahun]" value="{{$b->tahun}}" class="form-control {{$errors->first('tahun') ? "is-invalid" : ""}}" aria-describedby="basic-addon2">
                              </div>
